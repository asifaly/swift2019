...
// *****************************************************************
function Folder(folderDescription, hreference, decoration) //constructor
{
  //constant data
  ...
  this.nodeImg = 0;
  this.isLastNode = 0;
  this.iconSrc = ICONPATH + "fopenoff.gif";
  this.iconSrcOn = ICONPATH + "fopenon.gif";
  this.iconSrcClosed = ICONPATH + "fclosed.gif";
...
function drawFolder(insertAtObj)
{
  ...
  if (this.level>0)
    if (this.isLastNode) //the last child in the children array
      ...
      leftSide = leftSide + "<td valign=top background=" + ICONPATH + "tvline.gif>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=16 height=22 border=0></a></td>";
  ...
  {
    if (this.prependHTML == "")
        docW = docW + "<img src=" + ICONPATH + "tvblank.gif height=2 width=2>";
  }
  if (WRAPTEXT)
  ...
  if (USETEXTLINKS)
  {
    ...
    docW = docW + this.desc + "</a>" + this.deco ;
  }
  else
    docW = docW + this.desc + this.deco ;
  docW = docW + "</td>";
...
function linkFolderHTML(isTextLink)
{
  ...
  {
    if (USEFRAMES) {
      var targetFlag = this.hreference.charAt(0);
      if (targetFlag==".") {
        docW = docW + "<a href='" + this.hreference + "' TARGET=_top " ;
      }
      else {
        docW = docW + "<a href='" + this.hreference + "' TARGET=\"FRAME_BODY\" " ;
      }
    }
    else {
      docW = docW + "<a href='" + this.hreference + "' TARGET=_top " ;
...
function nodeImageSrc() {
  ...
  {
    if (this.nChildren == 0)
      srcStr = ICONPATH + "tvlast.gif" ;
    else
      if (this.isOpen)
        srcStr = ICONPATH + "tvmlast.gif" ;
      else
        srcStr = ICONPATH + "tvplast.gif" ;
  }
  else
  {
    if (this.nChildren == 0)
      srcStr = ICONPATH + "tvnode.gif" ;
    else
      if (this.isOpen)
        srcStr = ICONPATH + "tvmnode.gif" ;
      else
        srcStr = ICONPATH + "tvpnode.gif" ;
  }
  return srcStr;
}
...
// *************************************************************
function Item(itemDescription, decoration) // Constructor
{
  // constant data
...
function finalizeCreationOfItem(itemArray)
{
  ...
  itemArray.iconSrc = ICONPATH + "docoff.gif" ;
  itemArray.iconSrcOn = ICONPATH + "docon.gif" ;
...
function drawItem(insertAtObj)
{
  ...
  if (this.level>0)
    ...
    {
      leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "tvlast.gif' width=16 height=22></td>";
    }
    else
    {
      leftSide = leftSide + "<td valign=top background=" + ICONPATH + "tvline.gif><img src='" + ICONPATH + "tvnode.gif' width=16 height=22></td>";
    }
  ...
  docW = docW + "<tr>" + leftSide + "<td valign=top>";
  if (USEICONS)
      docW = docW + "<a " + fullLink  + " id=\"itemIconLink"+this.id+"\">" + "<img id='itemIcon"+this.id+ "' name='itemIcon" + this.id+"' " + "src='"+this.iconSrc+"' border=0>" + "</a>";
  else
    if (this.prependHTML == "")
        docW = docW + "<img src=" + ICONPATH + "tvblank.gif height=2 width=3>";
  ...
  if (USETEXTLINKS)
    docW = docW + "<a " + fullLink + " id=\"itemTextLink"+this.id+"\">" + this.desc + "</a>" + this.deco ;
  else
    docW = docW + this.desc + this.deco ;
...
function leftSideHTML(leftSideCoded) {
  ...
  for (i=0; i<leftSideCoded.length; i++)
  {
    ...
    {
      retStr = retStr + "<td valign=top background=" + ICONPATH + "tvline.gif><img src='" + ICONPATH + "tvline.gif' width=16 height=22></td>";
    }
    if (leftSideCoded.charAt(i) == "0")
    {
      retStr = retStr + "<td valign=top><img src='" + ICONPATH + "tvblank.gif' width=16 height=22></td>";
    }
  }
  return retStr;
}
...
function findObj(id)
{
  ...
  if (foldersTree == null)
    return null; // tree not yet loaded
...
function highlightObjLink(nodeObj) {
  if (!HIGHLIGHT || nodeObj==null || nodeObj.maySelect==false) {//node deleted in DB
    ...
    if (lastHighlighted != null && lastHighlighted.iconImg != null) {
       // switch OFF previous icon
      lastHighlighted.iconImg.src = lastHighlighted.iconSrc ;
    }
    if (nodeObj.iconImg && nodeObj.iconSrcOn) {
      // switch ON new icon
      nodeObj.iconImg.src = nodeObj.iconSrcOn ;
      lastHighlighted = nodeObj;
    }
    else {
...
function gFld(description, hreference, decoration)
{
  folder = new Folder(description, hreference, decoration);
  return folder;
}

function gLnk(optionFlags, description, linkData, decoration)
{
  ...
  newItem = new Item(description, decoration);
...
function preLoadIcons() {
  var auxImg;
  auxImg = new Image();
  auxImg.src = ICONPATH + "tvline.gif";
  auxImg.src = ICONPATH + "tvmlast.gif";
  auxImg.src = ICONPATH + "tvmnode.gif";
  auxImg.src = ICONPATH + "tvplast.gif";
  auxImg.src = ICONPATH + "tvpnode.gif";
  auxImg.src = ICONPATH + "tvblank.gif";
  auxImg.src = ICONPATH + "tvlast.gif";
  auxImg.src = ICONPATH + "tvnode.gif";
  auxImg.src = ICONPATH + "fclosed.gif";
  auxImg.src = ICONPATH + "fopenoff.gif";
  auxImg.src = ICONPATH + "docoff.gif";
  auxImg.src = ICONPATH + "fopenon.gif";
  auxImg.src = ICONPATH + "docon.gif";
}
...
// Cookie functions removed (PLB 2017-09-08).
